      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            sitemapStylesheet = data.toString().replace(siteRegex, _url2.default.resolve(siteUrl, _path.default.join(pathPrefix, indexOutput))); // Save the updated stylesheet to the public folder, so it will be
...
var getNodePath = function getNodePath(node, allSitePage) {
  ...
  var exactMatchRegex = new RegExp("^" + node.path.replace(/\/$/, "") + "$", "gi");
  var partialMatchRegex = new RegExp(node.path.replace(/\/$/, "") + "$", "gi");
  var partialMatch;
  ...
  for (var _iterator = _createForOfIteratorHelperLoose(allSitePage.edges), _step; !(_step = _iterator()).done;) {
    ...
    var _url = page.node.url.replace(/\/$/, "");

    if (exactMatchRegex.test(_url)) {
      node.path = page.node.url;
      return node;
    }

    if (partialMatchRegex.test(_url)) {
...
var addPageNodes = function addPageNodes(parsedNodesArray, allSiteNodes, siteUrl) {
  ...
  remainingNodes.forEach(function (_ref4) {
    ...
    addedPageNodes.pages.push({
      url: _url2.default.resolve(siteUrl, node.url),
      node: node
    });
  });
      ...
      if (currentSource) {
        ...
        currentSource.edges.map(function (_ref11) {
          ...
          sourceObject[mapping[type].sitemap].push({
            url: _url2.default.resolve(siteURL, node.path),
            node: node
          });
        });
      }
